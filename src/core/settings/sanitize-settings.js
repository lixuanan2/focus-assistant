...
import { DEFAULT_GROUP_ID, DEFAULT_SETTINGS } from "./defaults.js";
...
function sanitizeSchedule(schedule) {
  if (!schedule || typeof schedule !== "object") {
    return {
      enabled: DEFAULT_SETTINGS.schedule.enabled,
      windows: DEFAULT_SETTINGS.schedule.windows
    };
  }

  const rawWindows = Array.isArray(schedule.windows) ? schedule.windows : DEFAULT_SETTINGS.schedule.windows;
...
function sanitizeGroup(group, index) {
  ...
  return {
    ...
    name,
    modes: {
      permanent: typeof group.modes?.permanent === "boolean" ? group.modes.permanent : false,
...
function sanitizeBypassSettings(bypass) {
  return {
    ...
    allowFromBlockedPage: typeof bypass?.allowFromBlockedPage === "boolean"
      ? bypass.allowFromBlockedPage
      : DEFAULT_SETTINGS.bypass.allowFromBlockedPage
  };
}
